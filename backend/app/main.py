...
async def create_book_ep(payload: BookCreate, db: AsyncSession = Depends(get_session)):
...
async def list_books_ep(
	...
	items, total = await list_books(db, q=q, status=status, limit=page["limit"], offset=page["offset"])
	return {"items": items, "total": total, "limit": page["limit"], "offset": page["offset"]}


@app.get("/books/{book_id}", response_model=BookOut)
async def get_book_ep(book_id: int, db: AsyncSession = Depends(get_session)):
...
async def update_book_ep(book_id: int, payload: BookUpdate, db: AsyncSession = Depends(get_session)):
	book = await get_book(db, book_id)
	if not book:
		raise HTTPException(status_code=404, detail="Book not found")
...
async def delete_book_ep(book_id: int, db: AsyncSession = Depends(get_session)):
	book = await get_book(db, book_id)
	if not book:
		raise HTTPException(status_code=404, detail="Book not found")
...
async def set_status_ep(book_id: int, status: BookStatus, db: AsyncSession = Depends(get_session)):
	book = await get_book(db, book_id)
	if not book:
		raise HTTPException(status_code=404, detail="Book not found")
...
async def export_books_csv(db: AsyncSession = Depends(get_session)):
	items, _ = await list_books(db, q=None, status=None, limit=10_000, offset=0)
	return {"items": created, "total": len(created), "limit": len(created), "offset": 0}